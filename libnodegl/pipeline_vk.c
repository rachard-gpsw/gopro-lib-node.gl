...
static const VkCullModeFlagBits vk_cull_mode_map[NGLI_CULL_MODE_NB] = {
...
static VkCullModeFlagBits get_vk_cull_mode(int cull_mode)
{
    return vk_cull_mode_map[cull_mode];
}

static VkColorComponentFlagBits get_vk_color_write_mask(int mask)
{
    int bits = 0;
    if (mask & NGLI_COLOR_COMPONENT_R_BIT)
        bits |= VK_COLOR_COMPONENT_R_BIT;
    if (mask & NGLI_COLOR_COMPONENT_G_BIT)
        bits |= VK_COLOR_COMPONENT_G_BIT;
    if (mask & NGLI_COLOR_COMPONENT_B_BIT)
        bits |= VK_COLOR_COMPONENT_B_BIT;
    if (mask & NGLI_COLOR_COMPONENT_A_BIT)
...
static VkResult create_graphics_pipeline(struct pipeline *s, VkPipeline *pipeline_dstp)
{
    ...
    const struct graphicconfig *gc = &s->ctx->graphicconfig;
    ...
    VkPipelineRasterizationStateCreateInfo rasterization_state_create_info = {
        ...
        .lineWidth = 1.f,
        .cullMode = gc->cull_face ? get_vk_cull_mode(gc->cull_face_mode) : VK_CULL_MODE_NONE,
    ...
    VkStencilOpState stencil_op = {
        .failOp = get_vk_stencil_op(gc->stencil_fail),
        .passOp = get_vk_stencil_op(gc->stencil_depth_pass),
        .depthFailOp = get_vk_stencil_op(gc->stencil_depth_fail),
        .compareOp = get_vk_compare_op(gc->stencil_func),
        .compareMask = gc->stencil_read_mask,
        .writeMask = gc->stencil_write_mask,
        .reference = gc->stencil_ref,
    };

    VkPipelineDepthStencilStateCreateInfo depthstencil_state_create_info = {
        .sType = VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO,
        .depthTestEnable = gc->depth_test,
        .depthWriteEnable = gc->depth_write_mask,
        .depthCompareOp = get_vk_compare_op(gc->depth_func),
        .depthBoundsTestEnable = 0,
        .stencilTestEnable = gc->stencil_test,
        .front = stencil_op,
        .back = stencil_op,
        .minDepthBounds = 0.0f,
        .maxDepthBounds = 0.0f,
    ...
    VkPipelineColorBlendAttachmentState colorblend_attachment_state = {
        .blendEnable = gc->blend,
        .srcColorBlendFactor = get_vk_blend_factor(gc->blend_src_factor),
        .dstColorBlendFactor = get_vk_blend_factor(gc->blend_dst_factor),
        .colorBlendOp = get_vk_blend_op(gc->blend_op),
        .srcAlphaBlendFactor = get_vk_blend_factor(gc->blend_src_factor_a),
        .dstAlphaBlendFactor = get_vk_blend_factor(gc->blend_dst_factor_a),
        .alphaBlendOp = get_vk_blend_op(gc->blend_op_a),
        .colorWriteMask = get_vk_color_write_mask(gc->color_write_mask),
    };
